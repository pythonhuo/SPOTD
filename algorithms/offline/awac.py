...
class AdvantageWeightedActorCritic:
    ...
    def _actor_loss(self, states, actions):
        ...
        with torch.no_grad():
            ...
            # print("diff_loss", diff_loss.size())
            # exit()
            q = torch.min(
        ...
        diff_loss = self._diff.loss(pi_action, states)
        # print("diff_loss", diff_loss)
        # print("\n")
        # size = v.size()
        # device = diff_loss.device
        # diff_loss = torch.full(size, diff_loss, device=device)
        # adv_ls = torch.exp(adv / self._awac_lambda).mean()
        loss = adv.mean()/adv.abs().mean().detach() + diff_loss
        # print("diff_loss", diff_loss)
        # print("diff_loss", diff_loss.size())
        # print("loss", loss)
        ...
        # print("action_log_prob", action_log_prob.size())
        # # exit()
...
def train(config: TrainConfig):
    ...
    models = MLP(state_dim=state_dim, action_dim=action_dim, device=config.device)

    diff = Diffusion(state_dim=state_dim, action_dim=action_dim, model=models, max_action=max_action,
                          beta_schedule=beta_schedule, n_timesteps=T, ).to(config.device)

    diff.load_state_dict(torch.load('./diff_model_halfcheetah_medium-expert_b0.5_100000.pt'))
    diff.eval()
