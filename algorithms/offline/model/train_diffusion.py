...
# work dir
parser.add_argument('--work_dir', type=str, default='train_diffusion')
...
if args.model == 'Diffusion':
    diff = Diffusion(state_dim, action_dim, latent_dim, max_action, hidden_dim=args.hidden_dim).to(device)
else:
    raise NotImplementedError
optimizer = torch.optim.Adam(diff.parameters(), lr=args.lr, weight_decay=args.weight_decay)
if args.scheduler:
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=2000, eta_min=0.)
...
for step in tqdm(range(args.num_iters + 1), desc='train'):
    ...
    diff_loss = diff.loss(train_actions, train_states)

    # logger.log('train/recon_loss', recon_loss, step=step)
    # logger.log('train/KL_loss', KL_loss, step=step)
    logger.log('train/diff_loss', diff_loss, step=step)
    ...
    if step % 5000 == 0:
        logger.dump(step)
        torch.save(diff.state_dict(), '%s/diff_model_%s_%s_b%s_%s.pt' %
